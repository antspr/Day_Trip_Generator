...
if approval.upper == 'Y':
    ...
    new_selection = input("""What would you like to change? Choose "1" to change the destination, 
    "2" for a different place to eat 
    "3" for a different mode of travel 
    "4" for a different activity:""")
    if new_selection == 1:
      trip_package_list[0]= choose_destination(cities)
    elif new_selection == 2:
      trip_package_list[1]= choose_restaurant(restaurants)
    elif new_selection == 3:
      trip_package_list[2]= choose_vehicle(vehicles)
    elif new_selection == 4:
      trip_package_list[3]= choose_activity(activities)
